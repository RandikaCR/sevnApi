...
class ProductRangesController extends Controller
{
    private $screenPrefix = 'product_ranges';

    public function getProductRanges(Request $request){
        ...
        if (empty($isInvalid)){
            ...
            $uuId = !empty($request->product_range_id) ? $request->product_range_id : 0;

            $get = ProductRanges::select('product_ranges.*')
                ->when(!empty($uuId), function ($query) use ($uuId) {
                    ...
                    if (!empty($keyword)){
                        $query->where(function ($query) use ($keyword) {
                            return $query->where('product_range', 'like', '%'.$keyword.'%');
                        });
                    }
                    return $query;
                })
    ...
    public function setProductRange(Request $request){
        $out = [];
        ...
        if (empty($isInvalid)) {
            $getId = !empty($request->product_range_id) ? $request->product_range_id : 0;

            if (!empty($getId)){
                $validated = $request->validate([
                    'product_range' => 'required|unique:product_ranges,product_range,'.$getId .',uuid',
                    'label' => 'required',
                ]);

                $set = ProductRanges::where('uuid', $getId)->first();
                $set->product_range = $request->product_range;
                ...
                $out['message_text'] = 'Product Range has been Updated!';

            }else{
                $validated = $request->validate([
                    'product_range' => 'required|unique:product_ranges',
                    'label' => 'required',
                ]);

                $set = new ProductRanges();
                $set->product_range = $request->product_range;
                $set->label = !empty($request->label) ? $request->label : null;
                $set->image = !empty($request->image) ? $request->image : null;
                $set->status = 1;
                ...
                $update = ProductRanges::find($set->id);
                $update->uuid = $uuId;
                $update->save();
                ...
                $out['status'] = 'success';
                $out['message_title'] = 'Success!';
                $out['message_text'] = 'New Produt Range Added!';
